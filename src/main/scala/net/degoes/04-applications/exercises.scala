...
object exercises extends App {
  ...
  //use better monadic fold
  def crawlIO[E: Monoid, A: Monoid](
    seeds     : Set[URL],
    router    : URL => Set[URL], //normalize route or disgard
    processor : (URL, String) => IO[E, A]): IO[Nothing, Crawl[E, A]] = {
    def loop(seeds : Set[URL], visited : Set[URL]) : IO[Nothing, Crawl[E, A]] = {
      IO.traverse(seeds.flatMap(router))(
        url => getURL(url).redeem(
          _ => IO.now(mzero[Crawl[E, A]]),
          html =>
            for {
              crawl1 <- processor(url, html).redeemPure(Crawl(_, mzero[A]), a => Crawl(mzero[E], a))
  ...
  def crawlIO1[E: Monoid, A: Monoid](
    seeds     : Set[URL],
    router    : URL => Set[URL],
    processor : (URL, String) => IO[E, A]): IO[Nothing, Crawl[E, A]] = {
    def loop(seeds: Set[URL], crawl0: Crawl[E, A], visited: Set[URL]): IO[Nothing, Crawl[E, A]] =
        ...
        getURL(url).redeem(
          ...
          html =>
            processor(url, html).redeemPure(Crawl(_, mzero[A]), Crawl(mzero[E], _)).flatMap { crawl1 =>
              ...
              loop(urls -- visited, crawl0 |+| crawl1, visited ++ urls)
            }
...
//  }
  ...
  def crawlIORef[E: Monoid, A: Monoid](
    seeds     : Set[URL],
    router    : URL => Set[URL],
    processor : (URL, String) => IO[E, A]): IO[Nothing, Crawl[E, A]] = {
  ...
  //possible changes:
  // * use semaphore to limit concurrency -> limited parTraverse
  // * maybe use a queue and a consumer and limit number of consumers
  // * maybe pass a depth
  ...
  def crawlIOParQueue[E: Monoid, A: Monoid](
    seeds     : Set[URL],
    router    : URL => Set[URL],
  ...
  def gameLoop[F[_]: Console: Monad](state0: State): F[State] =
    for {
      ...
      state  <- if (state.playerLost)
        printLine[F]("Sorry, " + state.name + ", you lost!") *>
        printLine[F]("and the word was \""+ state.word + "\"") *> state.point[F]
  ...
  def getName[F[_]: Console: Apply]: F[String] =
    printLine[F]("Please enter your name:") *>
      readLine[F]
